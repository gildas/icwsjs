...
class Session {
  ...
  connect(server, user, password, application, language = 'en-US', marketPlace) {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then((data) => {
        ...
        // connect to station if any
        // start polling this session
        polled_sessions[this.id] = this;
  ...
  disconnect() {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then((data) => {
        if (polled_sessions[self.id] !== undefined) { polled_sessions[self.id] = undefined; }
...
/**
 * @description polls messages from a CIC server
**/
function poll_messages() {
  polled_sessions.forEach((session) => {
    new Rest(session).get('/messaging/messages')
    .then((data) => {
      session.log("Session" + session.id + ", Polling Results: " + data);
    })
    .catch((err) => {
      console.warn("Session " + session.id + 'Error: ' + err);
    });
  });
}
setInterval(poll_messages, POLLING_SLEEP * 1000);
